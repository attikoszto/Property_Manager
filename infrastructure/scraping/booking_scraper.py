...
class BookingScraper:
    ...
    async def scrape_listings(self) -> list[ScrapedListing]:
        logger.info(
            "Scraping Booking.com listings around (%.4f, %.4f) radius %.1f km",
            self.center_lat, self.center_lng, self.radius_km,
        )
        # TODO: Implement actual Booking.com scraping logic
        return []

    async def scrape_prices(self, listing_ids: list[str]) -> list[dict]:
        logger.info("Scraping Booking.com prices for %d listings", len(listing_ids))
